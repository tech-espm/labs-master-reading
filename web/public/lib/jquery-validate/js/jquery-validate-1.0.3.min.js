...
(function () {
	...
	$.validator.addMethod("componenteurl", function (value) {
		return (!value || !urlRegExp.test(value));
	}, "A URL deve conter apenas letras, números, e os caracteres \"-\" e \"_\".");

	$.validator.addMethod("aspectRatio", function (value) {
	...
	$.validator.addMethod("data", function (value, element) {
		...
		if (value.length > 10)
			return false;
		const b1 = value.indexOf("/");
		const b2 = value.lastIndexOf("/");
		if (b1 <= 0 || b2 <= b1)
			return value;
		const dia = parseInt(value.substring(0, b1));
		const mes = parseInt(value.substring(b1 + 1, b2));
		const ano = parseInt(value.substring(b2 + 1));
